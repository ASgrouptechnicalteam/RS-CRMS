    ...
    static async updateLeadStatus(user, leadId, newStatus, notes, guardFields) {
        ...
        return await p.$transaction(async (tx) => {
            ...
            if (newStatus === 'DEMO_SCHEDULED' && guardFields) {
                if (guardFields.demo_scheduled_at)
                    updateData.demo_scheduled_at = new Date(guardFields.demo_scheduled_at);
                if (guardFields.demo_handler_id)
                    updateData.demo_handler_id = guardFields.demo_handler_id;
            }
