...
class SiteVisitService {
    static async generateNextBookingCode() {
     ...
     */
    static async resolveVisitProject(data, companyId) {
    ...
    static async bookVisit(user, data) {
        ...
        if (!(0, authorization_1.can)(user, shared_2.Permissions.SITE_VISITS_CREATE, lead)) {
    ...
    static async acceptVisit(user, visitId, notes) {
        ...
        if (!visit)
            throw { status: 404, message: 'Site visit booking not found' };
        if (!(0, authorization_1.can)(user, shared_2.Permissions.SITE_VISITS_ASSIGN_AGENT, visit)) {
        ...
        return this.applyTransition(user, visitId, 'ACCEPT', { project_manager_id: user.employeeId }, 'SITE_VISIT_ACCEPTED', `Site visit ${visit.booking_code} accepted by ${user.employeeId}.${notes ? ` Notes: ${notes}` : ''}`);
    }
    ...
    static async reconfirmCustomer(user, visitId) {
        ...
        return this.applyTransition(user, visitId, 'RECONFIRM_CUSTOMER', {}, 'SITE_VISIT_REQUESTED', `Day-before reconfirmation call initiated for ${visit.booking_code}.`);
    }
    ...
    static async rescheduleVisit(user, visitId, data) {
        ...
        return this.applyTransition(user, visitId, 'RESCHEDULE', extra, 'SITE_VISIT_RESCHEDULE_REQUESTED', `Reschedule requested for ${visit.booking_code}.`);
    }
    ...
    static async pmReconfirm(user, visitId, release) {
        ...
        return this.applyTransition(user, visitId, 'PM_CONFIRM', {}, 'SITE_VISIT_ACCEPTED', `PM confirmed reschedule for ${visit.booking_code}.`);
    }
    ...
    static async confirmVisit(user, visitId) {
        ...
        return this.applyTransition(user, visitId, 'CONFIRM', {}, 'SITE_VISIT_ACCEPTED', `Site visit ${visit.booking_code} confirmed (schedule locked).`);
    }
    ...
    static async startVisit(user, visitId) {
        ...
        if (!visit)
            throw { status: 404, message: 'Site visit booking not found' };
        if (!(0, authorization_1.can)(user, shared_2.Permissions.SITE_VISITS_COMPLETE, visit)) {
            throw { status: 403, message: 'Forbidden: Missing site_visits.complete permission' };
        }
        return this.applyTransition(user, visitId, 'START', {}, 'SITE_VISIT_COMPLETED', `Site visit ${visit.booking_code} is now ACTIVE (in progress).`);
    }
    ...
    static async completeVisit(user, visitId, outcomes, feedback_notes, proof_photo_url) {
        ...
        const allNotInterested = outcomes.every((o) => o.outcome === 'NOT_INTERESTED');
        return await p.$transaction(async (tx) => {
            // Persist outcomes.
            for (const o of outcomes) {
