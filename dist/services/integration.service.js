...
exports.IntegrationService = exports.AppError = void 0;
...
class IntegrationService {
     ...
     */
    static async processPortalCallback(dto) {
     ...
     */
    static async processKycCallback(dto) {
     ...
     */
    static async processPaymentCallback(dto) {
    ...
    static parseIdempotencyKey(key) {
     ...
     */
    static istDateRangeFilter(from, to) {
        const range = {};
        if (from)
            range.gte = this.istMidnightUtc(from);
        if (to)
            range.lt = new Date(this.istMidnightUtc(to).getTime() + DAY_MS);
     ...
     */
    static normalizeCounts(known, groups, key) {
     ...
     */
    static async getPortalMetrics(companyId, query = {}) {
        ...
        const [handoffTotal, handoffGroups, eventTotal, eventTypeGroups, eventStatusGroups, retriedCount, terminalFailureCount, paymentTotal, paymentSyncGroups, paymentSourceGroups, customerTotal, customerKycGroups, kycSubmissions, notificationTotal, notificationTypeGroups,] = await Promise.all([
     ...
     */
    static async buildMetricsTimeseries(companyId, from, to) {
        const where = { company_id: companyId, ...this.istDateRangeFilter(from, to) };
        ...
        const days = new Map();
        const dayKey = (date) => (0, time_1.getISTComponents)(date).dateString;
