...
class PortalWorker {
    ...
    static async processHandoffEvent(event) {
        ...
        let response;
        try {
            response = await portalClient_1.PortalClient.sendHandoff(payload, idempotencyKey);
        }
        catch (err) {
    ...
    static async processKycStatusEvent(event) {
        ...
        let response;
        try {
            response = await portalClient_1.PortalClient.sendKycStatus(payload, idempotencyKey);
        }
        catch (err) {
            // Network error / timeout — retryable
    ...
    static async processPaymentStatusEvent(event) {
        let payload;
        try {
            payload = JSON.parse(event.payload);
        }
        ...
        let response;
        try {
            response = await portalClient_1.PortalClient.sendPaymentStatus(payload, idempotencyKey);
        }
        catch (err) {
            // Network error / timeout — retryable
    ...
    static async handlePaymentSuccess(event, body) {
        let payload;
        try {
            payload = JSON.parse(event.payload);
        }
    ...
    static async handlePaymentRetryableFailure(event, attempt, error) {
        let payload;
        try {
            payload = JSON.parse(event.payload);
        }
    ...
    static async handlePaymentTerminalFailure(event, error) {
        let payload;
        try {
            payload = JSON.parse(event.payload);
        }
    ...
    // ─────────────────────────────────────────────────────────────
    static async processInstallmentStatusEvent(event) {
        let payload;
        try {
            payload = JSON.parse(event.payload);
        }
        ...
        let response;
        try {
            response = await portalClient_1.PortalClient.sendInstallmentStatus(payload, idempotencyKey);
        }
        catch (err) {
            // Network error / timeout — retryable
    ...
    static async handleInstallmentSuccess(event, body) {
        let payload;
        try {
            payload = JSON.parse(event.payload);
        }
    ...
    static async handleInstallmentRetryableFailure(event, attempt, error) {
        let payload;
        try {
            payload = JSON.parse(event.payload);
        }
    ...
    static async handleInstallmentTerminalFailure(event, error) {
        let payload;
        try {
            payload = JSON.parse(event.payload);
        }
    ...
    static async handleSuccess(event, body) {
        await p.integrationEvent.update({
            where: { id: event.id },
            data: {
                status: 'COMPLETED',
    ...
    static async handleRetryableFailure(event, attempt, error) {
        await p.integrationEvent.update({
            where: { id: event.id },
            data: { error_message: error },
        });
    ...
    static async handleKycSuccess(event, body) {
        await p.integrationEvent.update({
            where: { id: event.id },
            data: {
                status: 'COMPLETED',
    ...
    static async handleKycRetryableFailure(event, attempt, error) {
        await p.integrationEvent.update({
            where: { id: event.id },
            data: { error_message: error },
        });
    ...
    static async handleKycTerminalFailure(event, error) {
        await p.integrationEvent.update({
            where: { id: event.id },
            data: {
                status: 'FAILED',
    ...
    static async handleTerminalFailure(event, error) {
        await p.integrationEvent.update({
            where: { id: event.id },
            data: {
                status: 'FAILED',
