...
class PaymentService {
    static async generateNextPaymentCode() {
    ...
    static async getPayments(user, bookingId) {
        const whereCondition = { company_id: user.companyId };
        if (bookingId) {
            whereCondition.booking_id = bookingId;
        }
        // Similar scope restrictions could be applied here if an agent queries all payments
        const isManagement = (user.roles || []).some((r) => ['Managing director', 'Admin (Technical)', 'HR', 'accountant', 'marketing director', 'Digital lead operator', 'project managers'].includes(r));
        if (!isManagement) {
    ...
    static async recordPayment(user, dto) {
    ...
    static async verifyPayment(user, id, status) {
        ...
        if (!payment_policy_1.PaymentPolicy.canMutate(user, payment)) {
        ...
        if ((user.roles || []).includes('Admin (Technical)') && !(user.roles || []).some((r) => ['Managing director', 'FINANCE', 'accountant'].includes(r))) {
        ...
        const result = await p.$transaction(async (tx) => {
                ...
                if (payment.installment_id) {
                    ...
                    if (newStatus !== installment.status) {
                        await tx.integrationEvent.create({
                            data: {
                                event_type: shared_1.INSTALLMENT_EVENT_TYPE,
            ...
            if (status === 'SUCCESS' || status === 'REFUNDED') {
                ...
                await tx.integrationEvent.create({
                    data: {
                        event_type: shared_1.PAYMENT_EVENT_TYPE,
                        payload: JSON.stringify({
                            event_type: shared_1.PAYMENT_EVENT_TYPE,
                ...
                if (status === 'SUCCESS') {
                    await notification_service_1.NotificationService.createCustomerNotificationTx(tx, {
                        company_id: user.companyId || 1,
