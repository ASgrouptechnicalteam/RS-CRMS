...
class BookingService {
    ...
    static async createBooking(user, dto, tx) {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                return await prisma_1.prisma.$transaction((client) => BookingService.claimAndCreate(client, user, dto));
            }
            catch (err) {
    ...
    `);
        ...
        if (property.status === 'LOCKED') {
            if (lockUntil && lockUntil >= now) {
                throw new lead_service_1.AppError(409, 'Property is currently locked');
            }
            // Expired lock is reclaimable.
        }
        else if (property.status === 'BOOKED' || property.status === 'SOLD') {
            throw new lead_service_1.AppError(409, 'Property has already been booked or sold');
        }
        else if (property.status !== 'LIVE') {
            throw new lead_service_1.AppError(409, 'Property is not available for booking');
        }
    ...
    static async cancelBooking(user, id, reason = 'Booking cancelled') {
        ...
        if (opp && opp.lead && opp.lead.status !== 'DROPPED') {
            await workflowEngine_1.WorkflowEngine.transition(p, opp.lead_id, 'DROPPED', { actor: user, entity: { ...opp.lead, exit_reason: reason } }, { exit_reason: reason });
        }
        return updated;
    }
}
