...
class PortalClient {
     ...
     */
    static async sendHandoff(payload, idempotencyKey) {
     ...
     */
    static async sendKycStatus(payload, idempotencyKey) {
        const portalApiUrl = process.env.PORTAL_API_URL || '';
        const crmPortalSecret = process.env.CRM_PORTAL_SECRET || '';
        if (!portalApiUrl) {
            throw new Error('PORTAL_API_URL is not configured');
     ...
     */
    static async sendPaymentStatus(payload, idempotencyKey) {
        const portalApiUrl = process.env.PORTAL_API_URL || '';
        const crmPortalSecret = process.env.CRM_PORTAL_SECRET || '';
        if (!portalApiUrl) {
            throw new Error('PORTAL_API_URL is not configured');
     ...
     */
    static async sendInstallmentStatus(payload, idempotencyKey) {
        const portalApiUrl = process.env.PORTAL_API_URL || '';
        const crmPortalSecret = process.env.CRM_PORTAL_SECRET || '';
        if (!portalApiUrl) {
            throw new Error('PORTAL_API_URL is not configured');
