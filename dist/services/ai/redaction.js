...
const SENSITIVE_VALUE_RE = /(api[_-]?key|pass(word)?|secret|token|jwt|authorization|credential|cvv|\bpan\b|aadhaar|aadhar|account[_-]?no\b|card[_-]?number\b|otp|pin\b)/i;
class Redactor {
    ...
    isSensitive(field) {
        ...
        if (SENSITIVE_FIELD_NAMES.some((name) => kind.includes(name)))
            return true;
