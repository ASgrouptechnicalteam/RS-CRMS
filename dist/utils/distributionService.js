 ...
 */
const findBestAssigneeForLead = async (companyId, preferredPmId) => {
    try {
        ...
        const telecallers = await p.employee.findMany({
            where: {
                ...
                roles: {
                    some: {
                        role: {
                            name: shared_1.Roles.TELECALLER,
                        },
                    },
                },
            },
        ...
        for (const emp of telecallers) {
            ...
            // 7-day call boost
            const callCountSum = emp.daily_reports?.reduce((sum, r) => sum + (r.call_count || 0), 0) || 0;
...
exports.findBestAssigneeForLead = findBestAssigneeForLead;
